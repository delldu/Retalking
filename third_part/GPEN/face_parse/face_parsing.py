...
class FaceParse(object):
    def __init__(self, base_dir='./', model='ParseNet-latest', device='cuda', 
        ...
        self.mfile = os.path.join(base_dir, model+'.pth') # 'checkpoints/ParseNet-latest.pth'
        self.size = 512
        self.device = device
        ...
        18: 'cloth'
        '''
        # no hair, no hat, no neck, no cloth
    ...
    def load_model(self):
        self.faceparse = ParseNet(self.size, self.size, 32, 64, 19, norm_type='bn', 
            relu_type='LeakyReLU', ch_range=[32, 256])
        self.faceparse.load_state_dict(torch.load(self.mfile, map_location=torch.device('cpu')))
        self.faceparse.to(self.device)
        self.faceparse.eval()

    # xxxx1111
    def process(self, im, masks):
        # im.shape -- (512, 512, 3)
