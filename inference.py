...
def main():    
    ...
    # enhancer -- <third_part.GPEN.gpen_face_enhancer.FaceEnhancement object>
    # enhancer.mask.shape -- (512, 512)
    # enhancer.process, enhancer.faceparser.process
    restorer = GFPGANer(model_path='checkpoints/GFPGANv1.3.pth', upscale=1, arch='clean', \
                        channel_multiplier=2, bg_upsampler=None)
    # restorer -- <third_part.GFPGAN.gfpgan.utils.GFPGANer object>
    # restorer.enhance

    base_name = args.face.split('/')[-1] # '1.mp4'
    ...
    if not os.path.isfile(args.face):
        ...
        while True:
            ...
            if not still_reading:
                ...
                break
            y1, y2, x1, x2 = args.crop # pp args.crop -- [0, -1, 0, -1]
            if x2 == -1: x2 = frame.shape[1]
            if y2 == -1: y2 = frame.shape[0]
            frame = frame[y1:y2, x1:x2]
            full_frames.append(frame)
    # len(full_frames) -- 135, full_frames[0].shape -- (h=700, w=700, 3)

    print ("[Step 0] Number of frames available for inference: "+str(len(full_frames)))
    # face detection & cropping, cropping the first frame as the style of FFHQ
    croper = Croper('checkpoints/shape_predictor_68_face_landmarks.dat')
    # croper -- <utils.ffhq_preprocess.Croper object>
    # croper.crop
    ...
    # full_frames_RGB[0].shape -- (419, 419, 3), crop -- (163, 0, 657, 469)
    # quad -- [38.06542863192817, 12.615428631928154, 457.1345713680719, 431.68457136807183]
    ...
    # frames_pil[0] -- <PIL.Image.Image image mode=RGB size=256x256>

    # get the landmark according to the detected face.
    # args.re_preprocess -- False
    if not os.path.isfile('temp/'+base_name+'_landmarks.txt') or args.re_preprocess:
        print('[Step 1] Landmarks Extraction in Video.')
        kp_extractor = KeypointExtractor()
        # kp_extractor -- <third_part.face3d.extract_kp_videos.KeypointExtractor object>
    ...
    if not os.path.isfile('temp/'+base_name+'_coeffs.npy') or args.exp_img is not None or args.re_preprocess:
        ...
        lm3d_std = load_lm3d('checkpoints/BFM') # lm3d_std.shape -- (5, 3)
    ...
    # semantic_npy.shape -- (135, 262)

    # generate the 3dmm coeff from a single image
    if args.exp_img is not None and ('.png' in args.exp_img or '.jpg' in args.exp_img): # False
    ...
    # load DNet, enet_model(LNet and ENet)
    dnet_model, enet_model = load_model(args, device)
    # dnet_model -- DNet(...)
    # enet_model -- ENet(...)
    # enet_model.low_res -- LNet(...)
    ...
    if not os.path.isfile('temp/'+base_name+'_stablized.npy') or args.re_preprocess:
        ...
        for idx in tqdm(range(len(frames_pil)), desc="[Step 3] Stablize the expression In Video:"):
            ...
            with torch.no_grad():
                output = dnet_model(source_img, coeff)
    ...
    # imgs.shape -- (135, 256, 256, 3)

    torch.cuda.empty_cache()

    if not args.audio.endswith('.wav'): # False
    ...
    while True:
        ...
        i += 1
    # len(mel_chunks) -- 109, mel_chunks[0].shape -- (80, 16)

    print("[Step 4] Load audio; Length of mel chunks: {}".format(len(mel_chunks)))
    imgs = imgs[:len(mel_chunks)]
    full_frames = full_frames[:len(mel_chunks)]  
    lm = lm[:len(mel_chunks)]
    # imgs[0].shape -- (256, 256, 3)
    # full_frames[0].shape -- (700, 700, 3)
    # lm.shape -- (109, 68, 2)
    ...
    if args.up_face != 'original': # True
        instance = GANimationModel()
        instance.initialize()
        instance.setup()
    # instance -- <third_part.ganimation_replicate.model.ganimation.GANimationModel object>
    ...
    for i, (img_batch, mel_batch, frames, coords, img_original, f_frames) in enumerate(tqdm(gen, desc='[Step 6] Lip Synthesis:', total=int(np.ceil(float(len(mel_chunks)) / args.LNet_batch_size)))):
        ...
        # img_batch.size() -- [16, 6, 384, 384]
        # mel_batch.size() -- [16, 1, 80, 16]
        # img_original.size() -- [16, 3, 384, 384]

        with torch.no_grad():
            incomplete, reference = torch.split(img_batch, 3, dim=1) 
            pred, low_res = enet_model(mel_batch, img_batch, reference)
            ...
            if args.up_face == 'original':
                ...
                test_batch = {'src_img': F.interpolate((img_original * 2 - 1), size=(128, 128), mode='bilinear'), 
                              'tar_aus': tar_aus.repeat(len(incomplete), 1)}
                instance.feed_batch(test_batch)
                instance.forward()
                cur_gen_faces = F.interpolate(instance.fake_img / 2. + 0.5, size=(384, 384), mode='bilinear')
                
            if args.without_rl1 is not False: # False
                incomplete, reference = torch.split(img_batch, 3, dim=1)
    ...
    # pp command --
    # ('ffmpeg -loglevel error -y -i examples/audio/1.wav -i temp/temp/result.mp4 '
    #  '-strict -2 -q:v 1 results/1_4.mp4')
...
def datagen(frames, mels, full_frames, frames_pil, cox):
    ...
    # frames_pil[0][0].shape  -- (68, 2)
    # frames_pil[0][1] -- <PIL.Image.Image image mode=RGB size=256x256>

    crops, orig_images, quads  = crop_faces(image_size, frames_pil, scale=1.0, use_fa=True)
    inverse_transforms = [calc_alignment_coefficients(quad + 0.5, [[0, 0], [0, image_size], [image_size, image_size], [image_size, 0]]) for quad in quads]
    # len(inverse_transforms) -- 109
    # inverse_transforms[0].shape -- (8,)

    del kp_extractor.detector

    oy1,oy2,ox1,ox2 = cox
    face_det_results = face_detect(full_frames, args, jaw_correction=True)
    # face_det_results[0][0].shape -- (285, 195, 3)
    # face_det_results[0][1] -- (105, 390, 295, 490)
    ...
    for i, m in enumerate(mels):
        ...
        if len(img_batch) >= args.LNet_batch_size: # args.LNet_batch_size -- 16
